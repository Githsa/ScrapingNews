...
// Index Page when visit the site
...
router.get('/scrape', function(req, res) {
  ...
  // Get info of the html site with request
  request('http://www.ocregister.com/', function(error, response, html) {

    // Now load html into cheerio and save it to $ for a shorthand selector
    var $ = cheerio.load(html);

    // This is an error handler for the website 
    var titlesArray = [];
    ...
    $('article .inner').each(function(i, element) {
        ...
        result.link = 'http://www.ocregister.com/' + $(this).children('header').children('h2').children('a').attr('href').trim();
          ...
          if(titlesArray.indexOf(result.title) == -1){
            ...
            Article.count({ title: result.title}, function (err, test){
              ...
              // Log that scrape is working, just the content was already in the Database
                ...
                console.log('spare database content')
              }
        ...
        // Log that scrape is working
          ...
          console.log('OC Register information not saved.')
        }
